...
def handle_opts():
    parser = argparse.ArgumentParser(
        ...
        description='Glens Nmap XML Merger gnxmerge - Merges <host> sections '
        'from multiple Nmap XML files',
        usage='%(prog)s -sources=./file.xml,file2.xml,/path/to/files')
    parser.add_argument('-s', '-sources', action='store', dest='sources',
                        required=True,
                        help='Comma separated list of paths to files and/or '
                        'folder contents to merge. An .xml extension is not '
                        'mandatory; all files and contents of target folder '
                        'will be (non-recursively) processed, regardless of '
                        'extension. If files are present which are not '
                        'valid XML, they will be skipped with warnings.')
    parser.add_argument('-v', '--version', action='version',
                        version='%(prog)s 1.0.1 : Glens Nmap XML Merger '
                        '(http://bitbucket.org/memoryresident/gnxtools)')
...
def start_xml(script_start_time):
    ...
    ET.SubElement(nmaprun, 'verbose', nmaprun_verbose_attribs)
    ET.SubElement(nmaprun, 'debug', nmaprun_debug_attribs)
...
def finalise_xml(nmaprun_merged_results, script_start_time):
    ...
    summary_string = ('Nmap XML merge done at ' + time.strftime("%c") + "; "
                      + str(total_hosts) + ' total hosts found in '
                      + str(total_files) + ' files; Merge completed in '
                      + str(total_script_time) + ' seconds')
    ...
    ET.SubElement(runstats, 'finished', finished_attribs)
    ET.SubElement(runstats, 'hosts', hosts_attribs)

    return nmaprun


def merge_hosts(nmaprun, file_list):
    # Iterate through each file in the target folder or each file passed as an
    # option.
        ...
        try:
            ...
            for current_host in current_nmap_file_blob.findall('host'):
                ...
                total_seconds = (total_seconds +
                                 calc_seconds(current_host.attrib['starttime'],
...
def output_results(nmap_file_preamble, nmaprun, merge_job_output):
    ...
    print(str(ET.tostring(nmaprun), "utf-8"))
    for badfile in bad_file_list:
        # Throw a warning to stderror but dont interfere with the working
        # output.
        # There might be a bunch of files we dont care about in the same
        # folder, such as nmap 'normal' outputs.
        sys.stderr.write("\n WARNING:" + badfile + " skipped (not xml?)")
    # Print general end status
    sys.stderr.write("\n\nMerged " + str(total_hosts) + " hosts from "
                     + str(files_processed) + " xml files. "
                     + str(len(bad_file_list)) + " invalid files skipped.")


def main():

    # Running time.time() through int as the former returns epoch in the form
    # of a float which is not the official format.
    # Discovered this because ndiff was breaking on the merged output. Also
    # applied to time.time call in finalise_xml
    script_start_time = int(time.time())
    merge_job_output = []

    nmap_file_preamble = ('<?xml version="1.0"?> \n'
                          '<!DOCTYPE nmaprun PUBLIC "-//IDN nmap.org//DTD '
                          'Nmap XML 1.04//EN" '
                          '"https://svn.nmap.org/nmap/docs/nmap.dtd"> \n'
                          '<?xml-stylesheet '
                          'href="https://svn.nmap.org/nmap/docs/nmap.xsl" '
                          'type="text/xsl"?> \n'
                          '<!-- nmap results file generated by gnxmerge.py: '
                          'Glens Nmap XML Merger '
                          'https://bitbucket.org/memoryresident/gnxtools -->'
                          '\n\n')
